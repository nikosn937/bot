...
import pandas as pd
import gspread
from datetime import datetime, timedelta # Προσθήκη timedelta για υπολογισμό ημερομηνιών
import re 
...
if selected_school and selected_school != "-- Επιλέξτε --" and not full_df.empty:
    ...
    if not current_tmimata:
        ...
        # ----------------------------------------------------------------------
        # ΝΕΑ ΛΟΓΙΚΗ: ΕΜΦΑΝΙΣΗ ΤΕΛΕΥΤΑΙΩΝ 2 ΗΜΕΡΩΝ ΠΡΙΝ ΤΗΝ ΑΝΑΖΗΤΗΣΗ
        # ----------------------------------------------------------------------
        ...
        recent_posts = filtered_df[filtered_df['Date'] >= two_days_ago.date()]
        ...
        # εμφανίζεται μόνο αν δεν υπάρχουν πρόσφατες ανακοινώσεις
        ...
        #st.info(info_message) # Αφαιρέθηκε, καθώς το μήνυμα είναι πλέον γενικό
        ...
        if user_input and keyword_to_data_map:
            ...
            if matching_keywords:
                ...
                # Φιλτράρισμα των αποτελεσμάτων αναζήτησης για να αποκλειστούν οι πρόσφατες
                # (Για να αποφύγουμε διπλοεμφάνιση, αν και η αναζήτηση keyword έχει νόημα για όλα)
                # Ωστόσο, είναι πιο απλό να εμφανίζουμε όλα τα αποτελέσματα που ταιριάζουν
                # για το keyword, ανεξάρτητα από την ημερομηνία.
                
                # ... (ΚΩΔΙΚΑΣ ΕΜΦΑΝΙΣΗΣ ΑΠΟΤΕΛΕΣΜΑΤΩΝ ΑΝΑΖΗΤΗΣΗΣ) ...
